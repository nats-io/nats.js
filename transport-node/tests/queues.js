...
describe(
  ...
  () => {
    it("queues - deliver to single queue", async () => {
    ...
    it("queues - deliver to multiple queues", async () => {
      const nc = await connect({ servers: u });
      const subj = createInbox();
      ...
      assert.equal(mc(subsa), 1);
      assert.equal(mc(subsb), 1);
      await nc.close();
    });

    it("queues - queues and subs independent", async () => {
      const nc = await connect({ servers: u });
      const subj = createInbox();
      const subs = [];
      let queueCount = 0;
